...
class LoginSerializer(serializers.Serializer) :
    ...
    otp = serializers.RegexField(regex=r"^\d{6}$" , required=False,error_messages={"invalid" : "Enter a valid 6-digits verification code."})
    ...
    def validate(self, attrs):
        ...
        if email :
            if not password :
                raise serializers.ValidationError({"password" : "Password is required for email login."})
        
        elif phone_number :
            if not otp :
                raise serializers.ValidationError({"otp" : "OTP is required for phone number login."})
            
            elif not verify_otp(phone_number , otp) :
                raise serializers.ValidationError("The provided verification code is invalid or has expired.")
